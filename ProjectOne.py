...
questionOne=""" SELECT FORMAT('\"%s\" %s %s %s', a.title, '-', l.count ,'Views') Results 
		FROM lastView l, articles a 
		WHERE l.newpath = a.slug 
		LIMIT 3"""

questionOneTwo="""SELECT FORMAT('%s %s %s %s', a.name, '-', COUNT(*), 'Views') Result 
		  FROM authors a 
...
questionOneThree="""SELECT FORMAT('%s %s %s%s %s', TO_CHAR(errors.date::DATE, 'Mon dd, yyyy'), '-', ROUND((CAST((errors.total*100) AS DECIMAL)/original.total),2),'%','errors') AS \"persinage of error\" 
                    FROM errors 
